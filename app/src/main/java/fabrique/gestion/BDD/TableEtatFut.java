...
public class TableEtatFut extends Controle {
    ...
    private TableEtatFut(Context contexte){
        ...
        for (tmp.moveToFirst(); !(tmp.isAfterLast()); tmp.moveToNext()) {
            etats.add(new EtatFut(tmp.getLong(0), tmp.getString(1), tmp.getInt(2), tmp.getInt(3), tmp.getInt(4) == 1));
        }
    ...
    public long ajouter(String texte, int couleurTexte, int couleurFond, boolean actif) {
        ...
        if (id != -1) {
            etats.add(new EtatFut(id, texte, couleurTexte, couleurFond, actif));
            Collections.sort(etats);
        }
        return id;
    }
