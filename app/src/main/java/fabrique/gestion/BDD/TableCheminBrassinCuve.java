...
public class TableCheminBrassinCuve extends Controle  {
    ...
    /**
     * Liste des noeuds du chemin du brassin dans la cuve
     */
     ...
     * @param contexte
     * @return instance de TableCheminBrassinCuve
     */
    ...
    /**
     * Fonction à utilisé pour ajouter un noeud au chemin du brassin dans la cuve
    ...
    /**
     * Fonction à utilisé pour modifier un noeud au chemin du brassin dans la cuve
    ...
    public void modifier(long id, long id_noeudAvecBrassin, long id_noeudSansBrassin) {
        ...
        if (accesBDD.update(nomTable, valeur, "id = ?", new String[] {"" + id}) == 1) {
            NoeudCuve noeud = recupererId(id);
    ...
    /**
     * Renvoi le premier noeud du chemin du brassin dans la cuve
     * en sachant que le premier noeud n'a pas de precedent
     * @return l'id du premier noeud du chemin du brassin dans la cuve
     */
    ...
    public void supprimer(long id) {
        ...
        if ((noeud.getId_noeudAvecBrassin() == -1) && (noeud.getId_noeudSansBrassin() == -1) && (accesBDD.delete(nomTable, "id = ?", new String[] {""+id}) == 1)) {
            noeuds.remove(noeud);
            NoeudCuve noeudPrecedent = recupererId(noeud.getId_noeudPrecedent());
     ...
     */
    private ArrayList<NoeudCuve> trierParId(ArrayList<NoeudCuve> liste, int petitIndex, int grandIndex) {
        int i = petitIndex;
        int j = grandIndex;
        // calculate pivot number, I am taking pivot as middle index number
        NoeudCuve pivot = liste.get(petitIndex+(grandIndex-petitIndex)/2);
     ...
     * @return string regroupant toutes les informations nécessaires à la sauvegarde du chemin du brassin dans la cuve
     */
    @Override
    public String sauvegarde() {
        StringBuilder texte = new StringBuilder();
        if (noeuds.size() > 0) {
            ArrayList<NoeudCuve> trierParId = trierParId(noeuds, 0, noeuds.size() - 1);
