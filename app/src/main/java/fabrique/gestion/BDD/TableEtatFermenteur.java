...
public class TableEtatFermenteur extends Controle {
    ...
    private TableEtatFermenteur(Context contexte){
        ...
        for (tmp.moveToFirst(); !(tmp.isAfterLast()); tmp.moveToNext()) {
            etats.add(new EtatFermenteur(tmp.getLong(0), tmp.getString(1), tmp.getString(2), tmp.getInt(3), tmp.getInt(4), tmp.getInt(5) == 1, tmp.getInt(6) == 1));
        }
    ...
    public long ajouter(String texte, String historique, int couleurTexte, int couleurFond, boolean avecBrassin, boolean actif) {
        ...
        if (id != -1) {
            etats.add(new EtatFermenteur(id, texte, historique, couleurTexte, couleurFond, avecBrassin, actif));
            Collections.sort(etats);
        }
        return id;
    }
    ...
    public ArrayList<EtatFermenteur> recupererListeEtatsActifsAvecBrassin() {
        ArrayList<EtatFermenteur> listeEtatActif = new ArrayList<>();
        for (int i=0; i<etats.size(); i++) {
    ...
    public ArrayList<EtatFermenteur> recupererListeEtatsActifsSansBrassin() {
        ArrayList<EtatFermenteur> listeEtatActif = new ArrayList<>();
        for (int i=0; i<etats.size(); i++) {
            if (etats.get(i).getActif() && !etats.get(i).getAvecBrassin()) {
                listeEtatActif.add(etats.get(i));
